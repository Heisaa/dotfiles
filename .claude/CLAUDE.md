...
In a sandbox, tamper-protected paths that don't exist in the repo appear as
`/dev/null` char devices owned by `nobody`: shell rc files, `.gitconfig`,
`.gitmodules`, `.ripgreprc`, `.mcp.json`, `.idea`, `.vscode`, parts of `.claude/`.

- Not repo content. Don't read, edit, commit, or report them.
- `Permission denied` on read and `??` in `git status` are expected.
- Stage explicit paths; `git add .` errors on them.
- They exist only inside the sandbox. Don't delete or gitignore them.
