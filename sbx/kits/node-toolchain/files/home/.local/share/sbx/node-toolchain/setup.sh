...
# Runs on each container start, as a kit startup step. Everything here is
...
# uid 1000 by default; this kit explicitly selects root.
if [ "$(id -u)" -ne 0 ]; then
  exec sudo -n env SBX_NODE_VERSION="$NODE_VERSION" bash "$0" "$@"
fi
...
install_node() {
  local requested="$1" arch tarball sums version tmp b release_dir
  if [[ "$requested" =~ ^[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    ...
    release_dir="$DIST/latest-v$requested.x"
  else
    log "invalid Node version: $requested" >&2; return 1
  fi
  case "$(uname -m)" in
    x86_64)          arch=x64 ;;
  ...
  sums="$(curl -fsSL --retry 3 --retry-delay 2 "$release_dir/SHASUMS256.txt")"
  tarball="$(awk -v a="linux-$arch.tar.gz" '$2 ~ ("node-v.*-" a "$") {print $2; exit}' <<<"$sums")"
  [ -n "$tarball" ] || { log "no linux-$arch build in release $requested" >&2; return 1; }
  ...
  curl -fsSL --retry 3 --retry-delay 2 -o "$tmp/$tarball" "$release_dir/$tarball"
